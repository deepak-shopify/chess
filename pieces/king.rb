require_relative 'direction'
require_relative '../internal_position'

class King < Piece
  UNICODE_BY_COLOR = {
    Piece::WHITE => '♔',
    Piece::BLACK => '♚',
  }

  INITIAL_POSITIONS_BY_COLOR = {
    Piece::WHITE => [
      InternalPosition.from_chess_pos('e', 1),
    ],
    Piece::BLACK => [
      InternalPosition.from_chess_pos('e', 8),
    ]
  }

  def get_unicode
    UNICODE_BY_COLOR[@color]
  end

  def valid_move?(move, piece_at_destination, is_path_blocked)
    diagonal_move = move.row_change.abs == 1 && move.col_change.abs == 1
    only_one_changed = move.row_change.abs == 1 || move.col_change.abs == 1
    diagonal_move || only_one_changed
  end

  def clear_paths(move)
    []
  end

  def self.get_initial_positions(color)
    INITIAL_POSITIONS_BY_COLOR[color]
  end
end
